...
int main(void) {
	...
	while(1) {
		...
		/*WARNING: If option 0, 1 or 3 is selected, the card may not be detected by PC/Laptop again, as it disturbs the FAT format, and you may have to format it again with FAT32. This options are given for learning the raw data transfer to & from the SD Card*/
		...
		totalBlocks = 1;
		#ifndef FAT_TESTING_ONLY
		if((option == 0x30) || (option == 0x33) || (option == 0x34)) {
			//get total number of blocks for options 0, 3 or 4
			TX_NEWLINE;
			TX_NEWLINE;
		...
		switch (option) {
			case '0':
				//error = SD_erase (block, totalBlocks);
			...
			case '3':
				...
				break;
			case '4':
				error = SD_readMultipleBlock (startBlock, totalBlocks);
				TX_NEWLINE;
				if(error) {
					transmitString_F(PSTR("Read failed.."));
				} else {
			...
			case '8':
				...
				for(i=0; i<13; i++) {
					fileName[i] = 0x00;
					//clearing any previously stored file name
				}
				i=0;
				while(1) {
					data = receiveByte();
					...
					if(data <0x20 || data > 0x7e) {
						continue;
						//check for valid English text character
					}
					transmitByte(data);
					fileName[i++] = data;
